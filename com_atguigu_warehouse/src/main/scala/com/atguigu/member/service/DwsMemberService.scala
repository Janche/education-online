...
object DwsMemberService {
  ...
  def importMemberUseApi(sparkSession: SparkSession, time: String) = {
    import sparkSession.implicits._ //隐式转换
    val dwdMember = DwdMemberDao.getDwdMember(sparkSession).where(s"dt='${time}'") //主表用户表
...
//    resultData.show()
      ...
      dayResult.union(historyResult).groupByKey(item => item.uid + "_" + item.dn)
          .mapGroups{case (key, iters) => {
              ...
              val uid = keys(0)
              val dn = keys(1)
              val list = iters.toList.sortBy(item => item.start_time) // 对开始时间排序
